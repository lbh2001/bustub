 ...
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    if (matrixA->GetRowCount() == matrixB->GetRowCount() && matrixA->GetColumnCount() == matrixB->GetColumnCount()) {
      ...
      std::unique_ptr<RowMatrix<T>> matrix = std::make_unique<RowMatrix<T>>(rows, cols);
      for (int i = 0; i < rows; i++) {
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    if (matrixA->GetColumnCount() == matrixB->GetRowCount()) {
      int m_rows = matrixA->GetRowCount();
      int m_cols = matrixB->GetColumnCount();
      std::unique_ptr<RowMatrix<T>> matrix = std::make_unique<RowMatrix<T>>(m_rows, m_cols);
  ...
  static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB,
    ...
    int ab_rows = matrixA->GetRowCount();
    int ab_cols = matrixB->GetColumnCount();
    if (a_cols != b_rows || ab_rows != matrixC->GetRowCount() || ab_cols != matrixC->GetColumnCount()) {
      return std::unique_ptr<RowMatrix<T>>(nullptr);
    }
